...
## MISE À JOUR v3 — à faire avant de tester

### 1) Réparer l'envoi de photo (obligatoire)
...
Sans ces règles, la photo est refusée ("row-level security policy").

### 2) Ce qui a changé dans le code
- Messages d'erreur traduits en français (fini "Invalid login credentials").
- Inscription : nom d'utilisateur + téléphone (facultatif).
- Profil : limite photo 2 Mo, mot de passe ACTUEL requis avant le nouveau,
  champ téléphone, et gestion propre des comptes Google (pas de mot de passe).

### À savoir (honnêteté)
- **SMS / vérification par téléphone** : pas inclus. Ça nécessite un prestataire
  SMS payant (type Twilio). Le numéro est donc stocké comme simple info, non vérifié.
  Ta 2FA par authenticator (déjà en place) est gratuite et plus sûre qu'un SMS.
- **Unicité du nom d'utilisateur** : le nom est enregistré, mais son unicité n'est
  PAS encore garantie (deux personnes pourraient prendre le même). Pour l'imposer
  vraiment, il faut une table `profiles` en base avec contrainte d'unicité —
  c'est justement la prochaine étape (passage des données dans la base).
